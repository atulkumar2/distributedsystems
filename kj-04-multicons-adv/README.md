...
**Stage 4** — Full-stack vehicle telemetry Kafka platform. One portal hub, five browser UIs, two stream-processing
...
[Alert UI :9503]             [Storage UI :9504]
        ...
        ▼
┌─────────────────────────┐       [DLQ Viewer :9505]
 │  vehicle-telemetry-dlq  │ ─────► original event + error + offset
 └─────────────────────────┘
```
...
Invalid payloads go straight to the DLQ, while transient processing failures retry with backoff
before the event is finally routed to the DLQ.

#### Dead-Letter Queue (DLQ)

Events that fail validation immediately, or exhaust retry attempts after transient
processing failures, are published to `vehicle-telemetry-dlq` with an error reason attached as metadata.
...
├── run.sh
├── docker-compose.yml       ← all 5 app services attached to the shared Kafka network
...
├── dlq-viewer/              ← Spring Boot UI :9505 — failed-event dashboard for the DLQ
...
Stage-specific ports and container names live in `.env` in this folder.

Shared infra values such as Postgres host port, shared network name, and shared container names live in `../infra/.env`.

Bootstrap both env files from the committed templates before first run:
...
- **DLQ Viewer UI** at <http://localhost:9505> — inspect failed events with error reason, partition, and offset
...
| DLQ record details without Kafka UI | <http://localhost:9505> → original event, error, timestamp, partition, offset |
